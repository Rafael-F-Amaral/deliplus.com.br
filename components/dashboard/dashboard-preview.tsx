...
function DesktopOrders() {
  ...
  return <div className="min-h-dvh bg-[#fafaf8] text-[#2d2f2a]"><div className="lg:flex"><Sidebar activeLabel="Pedidos" /><main className="min-w-0 flex-1"><header className="flex min-h-[106px] items-center justify-between border-b border-[#efeee9] bg-white px-8 lg:px-10"><div><h1 className="text-[34px] font-semibold tracking-[-.055em] text-[#252722]">Pedidos</h1><p className="mt-2 flex items-center gap-2 text-[13px] text-[#676961]"><span className="size-2 rounded-full bg-[#7cc65a] shadow-[0_0_0_4px_rgba(124,198,90,.11)]" />Aberta agora</p></div><div className="flex items-center gap-3"><button className="flex items-center gap-2 rounded-lg border border-[#ddddd8] bg-white px-5 py-3 text-[13px] font-medium text-[#4a4c45]"><Glyph name="pause" className="size-5" />Pausar pedidos</button><button className="flex items-center gap-2 rounded-lg bg-[#2b391e] px-5 py-3 text-[13px] font-medium text-white"><Glyph name="bag" className="size-5" />Abrir pedidos</button></div></header><div className="p-6 lg:p-10"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-[14px] text-[#676961]">Acompanhe, aceite e organize tudo em um só lugar.</p></div><div className="flex gap-2"><button className="flex items-center gap-2 rounded-lg border border-[#ddddd8] bg-white px-4 py-3 text-[13px] text-[#4a4c45]"><Glyph name="calendar" className="size-4" />Hoje <Glyph name="chevron" className="size-3.5" /></button><button className="rounded-lg border border-[#ddddd8] bg-white px-4 py-3 text-[13px] text-[#4a4c45]">Todos os status <Glyph name="chevron" className="ml-2 inline size-3.5" /></button></div></div><section className="mt-6 grid gap-4 sm:grid-cols-3"><MetricCard label="Novos pedidos" value="8" comparison="+12% vs ontem" icon="bag" /><MetricCard label="Em preparo" value="12" comparison="+8% vs ontem" icon="clock" /><MetricCard label="Pedidos hoje" value="124" comparison="18% vs ontem" icon="orders" /></section><section className="mt-6 grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><div className="rounded-xl border border-[#e9e8e3] bg-white p-6 shadow-[0_2px_10px_rgba(41,53,31,.02)]"><div className="flex items-center justify-between"><h2 className="text-[18px] font-semibold tracking-[-.035em]">Fila de pedidos</h2><button className="rounded-lg border border-[#e2e2dc] px-4 py-2 text-[13px] text-[#bd6842]">Exportar</button></div><div className="mt-5 space-y-2">{liveOrders.map((order, index) => <button type="button" key={order.id} onClick={() => setSelected(index)} className={`grid w-full grid-cols-[90px_minmax(0,1fr)_82px_112px] items-center gap-3 rounded-lg border px-4 py-4 text-left transition ${selected === index ? "border-[#c66c45] bg-[#fffaf6]" : "border-[#efeee9] hover:bg-[#fafaf8]"}`}><strong className="text-[14px]">{order.id}</strong><span className="truncate text-[14px] text-[#4e504a]">{order.customer}</span><span className="text-[13px] text-[#4e504a]">{order.time}</span><span className={`justify-self-end rounded-full px-3 py-1.5 text-[12px] font-medium ${order.tone}`}>{order.status}</span></button>)}</div></div><aside className="rounded-xl border border-[#e9e8e3] bg-white p-6 shadow-[0_2px_10px_rgba(41,53,31,.02)]"><div className="flex items-start justify-between"><div><p className="text-[13px] text-[#777970]">Pedido selecionado</p><h2 className="mt-1 text-[25px] font-semibold tracking-[-.045em]">{liveOrders[selected].id}</h2></div><span className={`rounded-full px-3 py-1.5 text-[12px] font-medium ${liveOrders[selected].tone}`}>{liveOrders[selected].status}</span></div><div className="mt-6 border-t border-[#efeee9] pt-5"><p className="text-[16px] font-semibold">{liveOrders[selected].customer}</p><p className="mt-2 text-[13px] text-[#666960]">Pedido feito às {liveOrders[selected].time} · entrega</p><div className="mt-5 space-y-3 text-[14px] text-[#555951]"><div className="flex justify-between"><span>1x Bowl Frango Grelhado</span><strong>R$ 34,00</strong></div><div className="flex justify-between"><span>1x Suco Natural Laranja</span><strong>R$ 12,00</strong></div><div className="flex justify-between"><span>Taxa de entrega</span><strong>R$ 18,00</strong></div></div><div className="mt-5 flex justify-between border-t border-[#efeee9] pt-4 text-[16px] font-semibold"><span>Total</span><span>R$ 64,00</span></div><div className="mt-6 grid gap-3"><button className="rounded-lg bg-[#39492a] py-3 text-[14px] font-semibold text-white">Aceitar pedido</button><button className="rounded-lg border border-[#c66c45] py-3 text-[14px] font-semibold text-[#b75f3b]">Rejeitar pedido</button></div></div></aside></section></div></main></div></div>
}

export function DashboardPreview({ initialView }: { initialView?: "overview" | "orders" | "menu" }) {
  if (initialView === "orders") return <OrdersDesktop />
